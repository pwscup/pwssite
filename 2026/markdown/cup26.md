...
## What's New
- 2026/09/12(土) 本戦の [salt](#salt) と[参加チーム・コホート割当](#team)を公開し、[キットの版](#releases)に本戦版を追加しました　★New
- 2026/09/09(水) [予備戦の結果](#prelim-results)と[本戦版のルールブック](#materials)を公開しました
...
| `main-process-20260912` | 本戦・加工フェーズ（2026/09/12 開始）向け |
...
本戦の salt は、本戦・加工フェーズのデータ配布（2026/09/12(土)）にともない公開します。

| ラウンド | salt |
| --- | --- |
| 予備戦 | `pwscup2026-prelim-640835c70b0c16c5bae356b2` |
| 本戦 | `pwscup2026-final-db3399c62a5f8341a1648f19` |
...
本戦の salt も同じく、`printf '%s' 'pwscup2026-final-db3399c62a5f8341a1648f19' | shasum -a 256` が上表の本戦のコミットメントと一致します。
...
<table class="schedule">
  <thead>
    <tr>
      <th scope="col">登録順</th>
      <th scope="col">予備戦のコホート番号</th>
      <th scope="col">本戦のコホート番号</th>
...
- 表の **コホート番号** は、ルールブック §3.2 のとおり `SHA-256(チーム名 ∥ salt)` を整数にして `mod N`（N=30）で決めています。**その番号がすでに埋まっていたら、空くまで +1 でずらします。**予備戦と本戦では salt が違うので、同じチームでも番号が変わります。
- **ずらす処理は登録順に行う**ので、検算には登録順が要ります。そのために **登録順**の列を付けてあります。検算に必要な salt は、予備戦・本戦とも[上の欄](#salt)に掲載しています。
...
- 表の読み方: 指導者の欄の **`-`** は指導者を置いていないことを示します（指導者は任意の役職です）。予備戦のコホート番号の欄の **`-`** は、本戦から参加したチームです。`（非公開）` はその役職の方がいて氏名・所属を掲載しない欄、`代表者と同じ` は代表者が責任者を兼ねているチーム、空欄は未記入の欄です。
