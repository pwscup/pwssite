"""Tests for scripts/build.py (GFM-compatible Markdown engine)."""
...
class TestExtractTitle:
    def test_extracts_h1(self, tmp_path: Path):
        ...
        assert build_mod.extract_title(md) == "My Title"

    def test_extracts_first_h1(self, tmp_path: Path):
        md = tmp_path / "test.md"
        md.write_text("# First\n# Second\n", encoding="utf-8")
        assert build_mod.extract_title(md) == "First"

    def test_default_when_no_h1(self, tmp_path: Path):
        md = tmp_path / "test.md"
        md.write_text("## Not H1\nsome content\n", encoding="utf-8")
        assert build_mod.extract_title(md) == "PWS"
...
class TestBuildBody:
    def test_basic_conversion(self):
        toc, html = build_mod.build_body("# Hello\n\nParagraph\n")
...
class TestInsertTocAfterWhatsNew:
    def test_no_toc(self):
        ...
        result = build_mod.insert_toc_after_whats_new(html, "")
        assert result == html
    ...
    def test_inserts_after_whats_new(self):
        ...
        result = build_mod.insert_toc_after_whats_new(html, toc)
...
class TestBuildHeader:
    def test_japanese_template(self, template_dir_2026: Path):
        ...
        header = build_mod.build_header(target_dir, "index", "Test Title")
